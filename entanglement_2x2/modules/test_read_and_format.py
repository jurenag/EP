...
def file_to_numpyarray_test(filepath, N, separator='\t', rows_to_skip=0):
    '''This function takes:
    - filepath: (string) The path to a data file. Such file which must represent a (bidimensional) table of 
    real numbers whose rows length is homogeneous. 
    - N: (integer): Dimension of the hilbert space to which the density matrices in filepath must belong. In 
    other words, 2*N*N must match the number of columns within each row of the file whose path is filepath. 
    
    The function takes such table and transforms it into a numpy array whose entries type is float. The function 
    returns such array.'''
    ...
    output_array = pd.read_table(   filepath, sep=separator, header=None, 
                                    index_col=None, usecols=range(2*N*N), 
...
def reformat_density_matrices(input_array, rows=4, columns=8):
    ...
    <columns> entries match the first row of the density matrix which encompass <rows> complex entries. The first 
